class Grid {
  ...
  drawActiveNotes(img){
    ...
    fill(255);
    noStroke();
    for (let i=0;i<this.notePos.length/2;i++){
    ...
    for (let i=this.notePos.length/2;i<this.notePos.length;i++){
      for (let j=0;j<this.notePos[i].length;j++){
        let x = this.notePos[i][j].x;
        let y = this.notePos[i][j].y;
        if (this.noteState[i][j]>0) {
          ...
          fill(mix);
          let s = this.noteState[i][j];
          rect(x/2+x/2, y, this.noteSize*s, this.noteSize*s);
        }
        //rect(x, y, this.noteSize*s, this.noteSize*s);
        this.noteState[i][j]-=0.05;
        this.noteState[i][j]=constrain(this.noteState[i][j],0,1);
